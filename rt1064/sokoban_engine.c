...
void build_map_info(SokobanContext *ctx, const uint8_t *raw_map, uint8_t cls)
{
    ...
    while (unid_boxes > 0 || unid_goals > 0)
    {
            ...
            !(ctx->redundant_tnt > 0U && path.length > RECON_MAX_ROUTE_LENGTH))
        {
            ...
            recon_angle = recon_direction_to_angle(target_direction);
...
static bool pass(uint8_t startpoint, uint8_t endpoint, float error, const uint8_t *obstacles)
{
    ...
    if (start_x == end_x)
    {
        ...
        {
            if (obstacles[y_run * WIDTH + end_x])
            {
                return 0;
            }
        }
    ...
    if (start_y == end_y)
    {
        ...
        {
            if (obstacles[end_y * WIDTH + x_run])
            {
                return 0;
            }
        }
    ...
    for (int y = (int)end_y - 1; y <= (int)end_y + 1; y++)
    {
        if (y < 0 || y >= HEIGHT)
            continue;
        for (int x = (int)end_x - 1; x <= (int)end_x + 1; x++)
        {
            if (x < 0 || x >= WIDTH)
                continue;
